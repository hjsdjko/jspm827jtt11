				...
				<form class="layui-form filter" :style='{"padding":"10px 7% 0","margin":"0 0 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
					...
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						...
						<select name="bumen" id="bumen" lay-filter="bumen">
							...
							<option v-for="(item,index) in bumenOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>职位</div>
						<input type="text" :style='{"padding":"0 10px","margin":"0","outline":"none","borderColor":"#ddd","color":"#666","borderRadius":"0px","borderWidth":"1px","width":"140px","fontSize":"14px","lineHeight":"40px","borderStyle":"solid","height":"40px"}' name="zhiwei" id="zhiwei" placeholder="职位" autocomplete="off" class="layui-input">
					</div>
		...
		<script>
			...
			layui.use(['form', 'layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate', 'slider'], function() {
				...
				function pageList() {
			        ...
			        if (jquery('#zhiwei').val()) {
			          param['zhiwei'] = jquery('#zhiwei').val() ? '%' + jquery('#zhiwei').val() + '%' : '';
			        }



					// 获取列表数据
					http.request('zhiwei/list', 'get', param, function(res) {
