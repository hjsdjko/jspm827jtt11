	...
	<div id="main-container">
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item-img">
                    ...
                    <div style="display: flex;align-items: center;">
                        <div v-for="(item,index) in qiyefengmianImgList" :key="index" style="position: relative;">
                            <div style="position: absolute;right: 10px;top: 0;width: 32px;height: 32px;background: #fff;padding: 6px;font-size: 0;cursor: pointer;border-radius: 0 20px 0 0;" @click="qiyefengmianDelClick(index)">
					...
					<div class="upload btn-img">选择文件<input name="file" type="file" id="qiyefengmianupload" class="form-control-file"></div>
					<input name="qiyefengmian" id="qiyefengmian-input" type="hidden">
				</div>

				<div class="form-item">
					<label>企业简介</label>
					<textarea  name="qiyejianjie" class="form-control textarea" id="qiyejianjie" rows="3"></textarea>
				</div>
				<div class="form-item">
					<label>招聘职位</label>
					<input name="zhaopinzhiwei" id="zhaopinzhiwei" name="zhaopinzhiwei" class="form-control" placeholder="请输入招聘职位">
				</div>
				<div class="form-item">
					<label>职位要求</label>
					<textarea  name="zhiweiyaoqiu" class="form-control textarea" id="zhiweiyaoqiu" rows="3"></textarea>
				</div>
				<div class="form-item">
					<label>上班时间</label>
				...
				<div class="form-item">
					<label>人事工号</label>
					<input name="gonghao" id="gonghao" name="gonghao" class="form-control" placeholder="请输入人事工号">
				</div>
				<div class="form-item">
					<label>人事姓名</label>
					<input name="xingming" id="xingming" name="xingming" class="form-control" placeholder="请输入人事姓名">
				</div>
				<div class="form-item">
					<label>联系电话</label>
	...
	<script>
		...
		var vm = new Vue({
		  ...
		  beforeCreate: function(){
			...
			methods: { 
                qiyefengmianDelClick(index){
                    var mymessage = confirm("确定移除图片？");
                    if (mymessage == true) {
                        this.qiyefengmianImgList.splice(index,1)  
                    }
		...
		// 文件上传
		function upload(){
			$('#qiyefengmianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {				
					document.getElementById('qiyefengmian-input').setAttribute('value',"upload/"+data.result.file);
					if(document.getElementById('qiyefengmianFileName') != null){
						document.getElementById('qiyefengmianFileName').innerHTML = "上传成功!";
					}				
                    vm.qiyefengmianImgList.push("upload/"+data.result.file)
				}
			});
		}  
		//取消 
		...
		function submit() {
			...
			if(validform() ==true && compare() == true){
                ...
                if(vm.qiyefengmianImgList.length){
                    data.qiyefengmian = vm.qiyefengmianImgList.join(',')
                }
		...
		function dataBind(){
            ...
            if(ruleForm.qiyefengmian){
                vm.qiyefengmianImgList = ruleForm.qiyefengmian.split(',')
            }
			$("#qiyefengmian-input").val(ruleForm.qiyefengmian);
			$("#qiyejianjie").val(ruleForm.qiyejianjie);
			$("#zhaopinzhiwei").val(ruleForm.zhaopinzhiwei);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.qiyefengmian != null && res.data.qiyefengmian != ''){

                        vm.qiyefengmianImgList = res.data.qiyefengmian.split(',')
					}
					if(res.data != null && res.data != '' && res.data.qiyejianjie != null && res.data.qiyejianjie != ''){

						$("#qiyejianjie").val(res.data.qiyejianjie);
					}
					if(res.data != null && res.data != '' && res.data.zhaopinzhiwei != null && res.data.zhaopinzhiwei != ''){

						$("#zhaopinzhiwei").val(res.data.zhaopinzhiwei);
						$("#zhaopinzhiwei").attr('readonly','readonly');
					}
