...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑员工</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>员工管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑员工</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>部门</label>
					<select id="bumenSelect" name="bumen" class="form-control" onchange="bumenChange1('bumen')">
							<option value="">请选择部门</option>
					</select>	
				</div>	
				<div class="form-item">
					<label>职位</label>
					<select id="zhiweiSelect" name="zhiwei" class="form-control">
	...
	<script>
        ...
        function bumenChange1(conditionColumn) {
            $('#zhiweiSelect option').not('option:first').remove();
            if($('#bumenSelect option:selected').val()) {
                http("option/zhiwei/zhiwei?conditionColumn="+conditionColumn+"&conditionValue="+$('#bumenSelect option:selected').val(),"GET",{
                },(res)=>{
                    if(res.code == 0){
                        if(res.data != null){
                            zhiweiOptions = res.data;
                            var zhiweiSelect = document.getElementById('zhiweiSelect');
                            for(var i=0;i<zhiweiOptions.length;i++){
                                var zhiweiOption = document.createElement('option');
                                zhiweiOption.setAttribute('name','zhiweiOption');
                                zhiweiOption.setAttribute('value',zhiweiOptions[i]);
                                zhiweiOption.innerHTML = zhiweiOptions[i];
                                zhiweiSelect.appendChild(zhiweiOption);
                            }
                        }
                    }
                });
			...
			http("option/zhiwei/zhiwei","GET",{},(res)=>{
				if(res.code == 0){
					...
					for(var i=0;i<zhiweiOptions.length;i++){
						...
						zhiweiOption.setAttribute('value',zhiweiOptions[i]);
						zhiweiOption.innerHTML = zhiweiOptions[i];									
						if(ruleForm.zhiwei == zhiweiOptions[i]){
							zhiweiOption.setAttribute('selected','selected');
						}
						zhiweiSelect.appendChild(zhiweiOption);
					}
				}
		...
		function submit() {
                //获取表单数据前取消禁用
                $("#bumenSelect").attr('disabled',false);
                //获取表单数据前取消禁用
                $("#zhiweiSelect").attr('disabled',false);
		...
		function dataBind(){
			...
			var zhiweiOptions = document.getElementsByClassName("zhiweiOption");
			for(var zhiweiCount = 0; zhiweiCount < zhiweiOptions.length;zhiweiCount++){
				if(zhiweiOptions[zhiweiCount].getAttribute('value') == ruleForm.zhiwei){
					zhiweiOptions[zhiweiCount].setAttribute('selected','selected');
				}
			}
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.zhiwei != null && res.data.zhiwei != ''){
						...
						var zhiweiOptions = document.getElementsByClassName("zhiweiOption");
						for(var zhiweiCount = 0; zhiweiCount < zhiweiOptions.length;zhiweiCount++){
							if(zhiweiOptions[zhiweiCount].getAttribute('value') == res.data.zhiwei){
								zhiweiOptions[zhiweiCount].setAttribute('selected','selected');
							}
						}
					}
		...
		function readonly(){
            if(window.sessionStorage.getItem('role')!="管理员") {
                ...
                $("#bumenSelect").attr('disabled',true);
                $("#zhiweiSelect").attr('disabled',true);
            }
		}
