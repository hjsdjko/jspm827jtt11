...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑培训信息</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>编辑培训信息</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>培训日期</label>
					<input id="peixunriqi-input" class="date" name="peixunriqi" size="15" type="text" readonly placeholder="请选择培训日期">
				</div>
				<div class="form-item">
					<label>培训天数</label>
				...
				<div class="form-item">
					<label>培训目的</label>
					<textarea  name="peixunmude" class="form-control textarea" id="peixunmude" rows="3"></textarea>
				</div>
				<div class="form-item">
					<label>培训备注</label>
				...
				<div class="form-item-file" >
					...
					<div class="upload btn-file">
						插入图片
						<input id="peixunneirongupload" name="file" type="file">
					</div>
					<script id="peixunneirongEditor" type="text/plain"></script>
	...
	<script>
		...
		function upload(){
			...
			$('#peixunneirongupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('peixunneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('peixunneirongEditor').hasContents()){
				$('#peixunneirong-input').attr('value',UE.getEditor('peixunneirongEditor').getContent());
			}
		}
		...
		function dateTimePick(){
			...
			$("#peixunriqi-input").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.peixunneirong != null && res.data.peixunneirong != ''){
                        ...
                        var peixunneirongMes = '' + res.data.peixunneirong;
                        var peixunneirongUeditor = UE.getEditor('peixunneirongEditor');
                        peixunneirongUeditor.ready(function() {
                            peixunneirongUeditor.setContent(peixunneirongMes);
                        });
