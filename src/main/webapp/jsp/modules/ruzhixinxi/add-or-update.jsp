...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑入职信息</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>编辑入职信息</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>合同名称</label>
					<input name="hetongmingcheng" id="hetongmingcheng" name="hetongmingcheng" class="form-control" placeholder="请输入合同名称">
				</div>
				<div class="form-item">
					<label>入职时间</label>
					<input id="ruzhishijian-input" class="date" name="ruzhishijian" size="15" type="text" readonly placeholder="请选择入职时间">
				</div>
				<div class="form-item">
					<label>合同条约</label>
					<textarea  name="hetongtiaoyue" class="form-control textarea" id="hetongtiaoyue" rows="3"></textarea>
				</div>
				<div class="form-item-file">
					<label>合同文件</label>
					<div class="upload btn-file">选择文件<input name="file" type="file" class="form-control-file" id="hetongwenjianupload"></div>
					<div id="hetongwenjianFileName" class="text"></div>
					<input name="hetongwenjian" id="hetongwenjian-input" type="hidden">
				</div>
				<div class="form-item">
					<label>登记日期</label>
					<input id="dengjiriqi-input" class="date" name="dengjiriqi" size="15" type="text" readonly placeholder="请选择登记日期">
				</div>
				<div class="form-item">
					<label>工号</label>
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "ruzhixinxi/info/" + id,
		...
		// 文件上传
		function upload(){
			$('#hetongwenjianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {							
					document.getElementById('hetongwenjian-input').setAttribute('value','file/download?fileName='+data.result.file);
					if(document.getElementById('hetongwenjianFileName') != null){
						document.getElementById('hetongwenjianFileName').innerHTML = "上传成功!";
					}		
					$("#hetongwenjianImg").attr("src",data.result.file);												
				}
			});
				...
				if(crossrefid && crossuserid) {
					...
					http("ruzhixinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("ruzhixinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function dateTimePick(){
			...
			$("#ruzhishijian-input").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
			...
			$("#dengjiriqi-input").datetimepicker({ 
				minView: "month",
				autoclose: true,
				format:'yyyy-mm-dd',
				language:'zh-CN',
		...
		function dataBind(){
			...
			$("#hetongtiaoyue").val(ruleForm.hetongtiaoyue);
			$("#hetongwenjian-input").val(ruleForm.hetongwenjian);
			$("#hetongwenjianFileName").html(ruleForm.hetongwenjian);
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.hetongtiaoyue != null && res.data.hetongtiaoyue != ''){

						$("#hetongtiaoyue").val(res.data.hetongtiaoyue);
					}
					if(res.data != null && res.data != '' && res.data.hetongwenjian != null && res.data.hetongwenjian != ''){

						$("#hetongwenjian-input").val(res.data.hetongwenjian);
						$('#hetongwenjianFileName').val(res.data.hetongwenjian)
					}
